...
from detectron2_loss import ValidationLoss
            ...
            if len(line) > 2:
                ...
                obj = {
                    "bbox": [int((float(cx) - (float(w_) / 2)) * width),
                             ...
                             int(float(w_) * width),
                             int(float(h_) * height)],
                    "bbox_mode": BoxMode.XYWH_ABS, # converting from yolo format to XYWH format (difference in notation for location of detected object within the image)
...
def train(output_dir, data_dir, class_list_file, learning_rate, batch_size, iterations, checkpoint_period, device,
    ...
    # Create a custom validation loss object. Detectron2 does not give loss function data on validation set by defaul. This must be added by following code. (Custom output)
